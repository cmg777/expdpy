"""Tests for analyze_fwl_plot (Frisch-Waugh-Lovell residualized scatter)."""
...
from expdpy import FWLPlotResult, analyze_fwl_plot, analyze_regression_table
...
def test_fwl_is_conditional_on_fixed_effects(sample_df):
    ...
    fe = analyze_fwl_plot(sample_df, dv="x2", var="x1", feffects=["firm"])
    ...
    no_fe = analyze_fwl_plot(sample_df, dv="x2", var="x1")
...
def test_fwl_no_controls_matches_simple_ols(sample_df):
    ...
    res = analyze_fwl_plot(sample_df, dv="x2", var="x1")
...
def test_fwl_figure_structure_and_band(sample_df):
    ...
    res = analyze_fwl_plot(sample_df, dv="x2", var="x1", controls=["x3"])
...
def test_fwl_n_sample_reduces_points_not_slope(sample_df):
    ...
    full = analyze_fwl_plot(
        sample_df, dv="x2", var="x1", controls=["x3"], n_sample=None
    )
    sub = analyze_fwl_plot(sample_df, dv="x2", var="x1", controls=["x3"], n_sample=20)
...
def test_fwl_focal_var_in_controls_raises(sample_df):
    with pytest.raises(ValueError, match="must not also be a control"):
        analyze_fwl_plot(sample_df, dv="x2", var="x1", controls=["x1", "x3"])


def test_fwl_missing_column_raises(sample_df):
    with pytest.raises(KeyError, match="nope"):
        analyze_fwl_plot(sample_df, dv="x2", var="nope")
...
def test_fwl_export_emitter_resolves_default_focal():
    ...
    assert "analyze_fwl_plot" in code
    assert (
        "var='x1'" in code
...
def test_fwl_export_emitter_skips_without_regressors():
    ...
    code = "\n".join(c for _, c in build_blocks(cfg, ["fwl_plot"]))
    assert "analyze_fwl_plot" not in code
