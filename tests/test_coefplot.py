"""Tests for analyze_coefficient_plot (themed coefficient plots)."""
...
def test_accepts_regression_table_result(pooled):
    res = ex.analyze_coefficient_plot(pooled)
...
def test_accepts_single_model_and_list(pooled):
    one = ex.analyze_coefficient_plot(pooled.models[0])
    assert len(one.fig.data) == 1
    two = ex.analyze_coefficient_plot([pooled.models[0], pooled.models[0]])
    assert len(two.fig.data) == 2


def test_drop_intercept_default_and_override(pooled):
    default = ex.analyze_coefficient_plot(pooled)
    assert "Intercept" not in set(default.df["term"])
    kept = ex.analyze_coefficient_plot(pooled, drop_intercept=False)
    assert "Intercept" in set(kept.df["term"])


def test_keep_filters_terms(pooled):
    res = ex.analyze_coefficient_plot(pooled, keep=["log_gdp_pc_sq"])
    assert set(res.df["term"]) == {"log_gdp_pc_sq"}


def test_multi_model_labels_and_dodge(pooled, fe):
    res = ex.analyze_coefficient_plot(
...
def test_model_labels_length_mismatch_raises(pooled):
    with pytest.raises(ValueError, match="model_labels"):
        ex.analyze_coefficient_plot(pooled, model_labels=["only one", "too many"])


def test_joint_bands_at_least_as_wide_as_pointwise(pooled):
    point = ex.analyze_coefficient_plot(pooled, drop_intercept=False)
    joint = ex.analyze_coefficient_plot(pooled, drop_intercept=False, joint=True)
...
def test_alpha_widens_intervals(pooled):
    ci95 = ex.analyze_coefficient_plot(pooled, alpha=0.05)
    ci90 = ex.analyze_coefficient_plot(pooled, alpha=0.10)
...
def test_orientation_switches_error_axis(pooled):
    horizontal = ex.analyze_coefficient_plot(pooled, horizontal=True)
    vertical = ex.analyze_coefficient_plot(pooled, horizontal=False)
