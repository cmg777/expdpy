"""Tests for analyze_regression_table (OLS / FE / clustered SEs via pyfixest)."""
...
def test_ols_coefficients_match_statsmodels(sample_df):
    res = analyze_regression_table(sample_df, dvs="x2", idvs=["x1"])
...
def test_non_numeric_dv_raises():
    ...
    with pytest.raises(NotImplementedError):
        analyze_regression_table(df, dvs="y", idvs=["x"])


@pytest.mark.parametrize("fmt", ["gt", "md", "df"])
def test_format_variants(sample_df, fmt):
    res = analyze_regression_table(sample_df, dvs="x2", idvs=["x1"], format=fmt)
