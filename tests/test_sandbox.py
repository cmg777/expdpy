...
def test_ovb_short_is_biased_long_is_not():
    res = ex.sandbox_omitted_variable_bias(beta_x=1.0, beta_z=1.0, corr_xz=0.6, seed=0)
...
def test_ovb_bias_grows_with_correlation():
    low = ex.sandbox_omitted_variable_bias(corr_xz=0.2, seed=1).summary["bias"]
    high = ex.sandbox_omitted_variable_bias(corr_xz=0.8, seed=1).summary["bias"]
    assert abs(high) > abs(low)


def test_pooled_vs_fe_recovers_truth():
    res = ex.sandbox_pooled_vs_fixed_effects(beta=1.0, unit_effect_corr=0.8, seed=0)
    s = res.summary
...
def test_clustering_inflates_se_not_estimate():
    res = ex.sandbox_clustering_se(n_clusters=40, cluster_size=30, icc=0.4, seed=0)
    s = res.summary
