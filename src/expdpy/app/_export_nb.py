"""Generate a reproducible notebook/script of the current ExPanD analysis."""
...
def _emit_missing(cfg: dict, ts_id: str | None) -> str:
    return f"ex.prepare_missing_values_graph(df, ts_id={_q(ts_id)})"
...
def _emit_by_group_violin(cfg: dict) -> str:
    return (
        ...
        f"var={_q(cfg.get('bgvg_var'))})"
    )


def _emit_by_group_trend(cfg: dict, ts_id: str | None) -> str:
    return (
        f"ex.prepare_by_group_trend_graph(df, ts_id={_q(ts_id)}, "
        f"group_var={_q(cfg.get('bgtg_byvar'))}, var={_q(cfg.get('bgtg_var'))}).fig"
    )


def _emit_trend(cfg: dict, ts_id: str | None) -> str:
    variables = [
        cfg.get(k)
        for k in ("trend_graph_var1", "trend_graph_var2", "trend_graph_var3")
        if cfg.get(k) not in (None, "None")
    ]
    return f"ex.prepare_trend_graph(df, ts_id={_q(ts_id)}, var={_q(variables)}).fig"


def _emit_quantile_trend(cfg: dict, ts_id: str | None) -> str:
    qs = [float(q) for q in cfg.get("quantile_trend_graph_quantiles", [])]
    return (
        f"ex.prepare_quantile_trend_graph(df, ts_id={_q(ts_id)}, "
        f"var={_q(cfg.get('quantile_trend_graph_var'))}, quantiles={_q(qs)}).fig"
    )
...
def _emit_scatter(cfg: dict) -> str:
    ...
    return (
        ...
        f"loess={1 if cfg.get('scatter_loess') else 0})"
    )
...
def build_blocks(
    config: dict, components: Sequence[str], ts_id: str | None = None
) -> list[tuple[str, str]]:
...
def build_script(
    ...
    parts = ["# Generated by expdpy ExPanD — reproducible analysis script", ""]
    for heading, code in build_blocks(config, components, ts_id):
...
def build_notebook(
    ...
    nb.cells.append(
        nbformat.v4.new_markdown_cell("# ExPanD analysis\n\nGenerated by `expdpy`.")
    )
    for heading, code in build_blocks(config, components, ts_id):
...
def build_export_zip(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr("ExPanD_analysis.ipynb", build_notebook(config, components, ts_id))
        zf.writestr("ExPanD_analysis.py", build_script(config, components, ts_id))
    return buf.getvalue()
