...
Beyond the panel ids (:mod:`expdpy._panel`) and display labels (:mod:`expdpy._labels`), a
dataset usually has a *focus*: one variable is the **outcome** of interest and one or more are
...
Like :func:`~expdpy.set_panel` / :func:`~expdpy.set_labels`, the roles are stashed on the
...
be declared from a data dictionary (``df_def``) via ``set_labels(df, df_def, set_panel=True)``
...
def set_roles(
    ...
    *,
    ...
    The roles are stored under ``df.attrs["expdpy_roles"]`` so that subsequent
    ...
    -------
    pandas.DataFrame
        The same ``df``, with ``df.attrs["expdpy_roles"]`` updated.
