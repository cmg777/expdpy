...
The mapping can be supplied directly, or extracted from a data-definition frame (``df_def``):
for each variable the concise ``label`` column is used, falling back to the longer ``var_def``
description, and finally to the bare ``var_name``. Because a ``df_def`` also tags each column's
``type`` (entity / time / …), :func:`set_labels` can declare the panel in the same call via
``set_panel=True``.
...
def _mapping_from_def(df_def: pd.DataFrame) -> dict[str, str]:
    """Build a ``{var_name: label}`` mapping from a data-definition frame.
    ...
    """
    if "var_name" not in df_def.columns:
        raise ValueError("df_def must have a 'var_name' column")
    has_label = "label" in df_def.columns
    has_var_def = "var_def" in df_def.columns
...
def _panel_ids_from_def(df_def: pd.DataFrame) -> tuple[str | None, str | None]:
    """Extract the first ``entity`` and ``time`` column names from a ``df_def``."""
    if "type" not in df_def.columns:
        return None, None
    entities = list(df_def.loc[df_def["type"] == "entity", "var_name"])
    times = list(df_def.loc[df_def["type"] == "time", "var_name"])
...
def set_labels(
    ...
    *,
    ...
    The labels are stored under ``df.attrs["expdpy_labels"]`` so that subsequent
    ...
    df
        ...
        returned).
    labels
        Either a ``{column_name: label}`` mapping, or a data-definition frame (``df_def``)
    ...
    -------
    pandas.DataFrame
        The same ``df``, with ``df.attrs["expdpy_labels"]`` updated.

    Examples
    --------
    Declare labels once (from the bundled data dictionary), then explore with readable titles:
