...
- **LLM/agent layer** (the AI-friendly surface): `_meta.py` is the **single source of truth** —
  a private, shipped, SDK-free module that curates a subset of the public API as `ToolSpec`s and
  compiles a JSON Schema per tool *by introspection* (signature + NumPy docstring), exposing
  `param_schema` / `anthropic_schema` / `openai_schema` / `GUARDRAIL_PREAMBLE`. Three consumers
  share it and must never re-declare API facts: the **MCP server** `mcp/` (optional
  `expdpy[mcp]`, stdio, `expdpy-mcp`; tools return `.interpret()` + tables + a saved figure path),
  the static-schema emitter `tools/build_tool_schemas.py` (→ committed `schemas/*.json`), and the
  `tools/build_llms_txt.py` index. The end-user **`use-expdpy` skill**
  (`.claude/skills/use-expdpy/`, vs the `write-function` dev skill) and the hub page
  `docs/use-with-llms.qmd` are generated by `tools/build_use_skill.py`. Only curation membership,
  `ToolSpec` output-surface metadata, and hand-written skill/llms prose are authored; everything
  else is derived and drift-checked (the `docs.yml` `artifacts-fresh` job + pytest that every
  cited name is in `__all__`).
...
- **Keep the LLM/agent layer in sync.** Every new public function must reach the agent-facing
  surface: it auto-flows into `llms.txt` / the function catalog (you regenerate and commit the
  artifacts — the `artifacts-fresh` CI gate enforces it), and a high-value, headless-safe function
  should be curated as an MCP tool. See **Keeping the LLM layer in sync** below.
...
## Keeping the LLM layer in sync

The agent-facing surface — `llms.txt` / `llms-full.txt`, the per-page `.md` companions, the
`use-expdpy` skill's `function-catalog.md`, the Anthropic/OpenAI tool schemas, and the MCP server —
all derive from one source of truth: **`src/expdpy/_meta.py`** plus `__all__` and the live
docstrings. **Never hand-edit a generated artifact**; edit the source and regenerate. There are two
layers, and both must be considered whenever the public API changes:

- **Auto-derived — regenerating is mandatory.** Once a new public function is exported in `__all__`
  and registered in `docs/_quarto.yml` `quartodoc.contents` (the Core wiring step), it
  automatically flows into `llms.txt` / `llms-full.txt`, its per-page `.md`, and the skill
  `function-catalog.md`. Regenerate and commit, or the `docs.yml` **`artifacts-fresh`** CI job
  fails:
  ```bash
  ...
  ```
  then commit `schemas/anthropic_tools.json`, `schemas/openai_tools.json`,
  `.claude/skills/use-expdpy/references/function-catalog.md`, `docs/use-with-llms.qmd` and
  `docs/llms.txt`. (`llms-full.txt` and the per-page `.html.md` twins are `_site`-only build
  artifacts, written by the final `build_llms_txt.py` step after `quarto render` — not committed.)
  Each generator's `--check` mode mirrors the CI gate.
- **Curated — opt-in for high-value tools.** The MCP server and the function-calling schemas expose
  a *curated subset*, `CURATED_TOOL_SPECS` in `_meta.py`; a new function does **not** appear there
  automatically. If it is high-value and headless-safe, append a `ToolSpec(name="<fn>", …)` with
  its non-introspectable output-surface metadata only — `figure_attrs` / `tables` / `table_methods`
  / `see_also_topics` (registered explainer keys), plus `forced_kwargs={"format": "df"}` for
  `_table` functions. The input JSON Schema is **compiled from the live signature + docstring**, so
  never hand-write parameter types. `tests/test_meta.py` and `tests/test_tool_schemas.py` validate
  the spec, and `_meta.uncurated_analysis_functions()` (a test) lists everything not yet curated so
  additions are triaged, not silently dropped.

All agent-facing text stays **association-only** (never "causes" / "effect of"): the shared
`GUARDRAIL_PREAMBLE` and the name-existence / banned-phrasing tests enforce it.
...
- **Regenerate the committed LLM artifacts** after touching the public API, a curated function's
  docstring, the `_meta` registry, a `use-expdpy` `SKILL.md`/reference, or a navbar/section in
  `_quarto.yml`, or the `docs.yml` `artifacts-fresh` job fails. See **Keeping the LLM layer in
  sync** above for the exact commands and the commit list.
