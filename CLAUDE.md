...
- **Regenerate the committed LLM artifacts after touching the public API, a curated function's
  docstring, the `_meta` registry, a `SKILL.md`/reference, or a navbar/section in `_quarto.yml`:**
  ...
  then commit `schemas/*.json`, `.claude/skills/use-expdpy/references/function-catalog.md`,
  `docs/use-with-llms.qmd` and `docs/llms.txt` — otherwise the `docs.yml` `artifacts-fresh` job
  fails. `llms-full.txt` and the per-page `.html.md` twins are `_site`-only build artifacts
  (written by the final `build_llms_txt.py` step, after `quarto render`) and are not committed.
