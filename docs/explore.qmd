...
<div class="try-banner">
  ...
  <div class="try-banner__actions">
    <a class="xpd-btn xpd-btn--app" href="https://expdpy-explore.streamlit.app/" target="_blank" rel="noopener">🚀 Launch the Explore app</a>
    <a class="xpd-btn xpd-btn--colab" href="https://colab.research.google.com/github/cmg777/expdpy/blob/main/notebooks/explore.ipynb" target="_blank" rel="noopener">▶ Open in Colab</a>
  </div>
</div>
```
...
`time=` turns it into an animation: press **▶ Play** to watch each continent's distribution move
year by year (the value axis stays pinned so the motion is comparable).
...
only a handful of members. It animates over time the same way.
...
ex.explore_treemap_plot(
    df, path=["continent", "country"], size="population", color="gini_regional", time="year"
).fig
```
