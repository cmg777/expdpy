...
# "Regional inequality (Gini)" rather than the bare column name. With `set_panel=True` it
# also declares the panel (entity = country, time = year), so the calls below can omit
# `entity=` / `time=` and panel-aware tables summarize by period.
...
Basic — with the panel declared, each statistic is shown by period (first and last year):
