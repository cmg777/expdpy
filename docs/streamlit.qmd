---
title: "The ExPdPy Streamlit app"
---
...
`ExPdPy` ships as a [Streamlit](https://streamlit.io/) app — a no-code interface to the
library in a **multipage layout** with native, sortable tables, ready to deploy on
[Streamlit Community Cloud](https://streamlit.io/cloud). It is provided by the `streamlit`
extra:
...
| **Regression** | OLS with fixed effects and clustered standard errors — with plain-language interpretation, a method explainer and a coefficient plot |
| **Event study & DiD** | staggered difference-in-differences / event study and the treatment-structure panel view (shown for panel data) |
| **Panel models** | pooled / between / fixed / random effects and the Hausman test (panel data; needs the `panel` extra) |
| **Concept sandboxes** | simulated teaching demos — omitted-variable bias, pooled-vs-fixed-effects, clustering (always available) |

Time-series pages and components (Trends, missing values, by-group trend) are **hidden
automatically for cross-sectional data**.
...
(defaulting to Kuznets; also Gapminder) and a file uploader. An
...
::: {.callout-note}
Expressions are evaluated with a restricted AST walker — never `eval`/`exec` — which is
strictly safer than R's sandboxed `eval`.
:::
...
The `ExPdPy` launcher (`from expdpy.streamlit_app import ExPdPy`) accepts several customization
arguments.
