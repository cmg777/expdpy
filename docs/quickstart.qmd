...
df = load_kuznets()
df[["country", "region", "year", "gini_regional", "gdp_pc", "log_gdp_pc"]].head()
```
...
```{python}
ex.prepare_by_group_bar_graph(df, "region", "gini_regional", np.nanmedian).fig
```

```{python}
ex.prepare_by_group_violin_graph(df, "region", "gini_regional")
```

## Scatter plot with LOESS

The **N-shaped Kuznets curve** — regional inequality against (log) GDP per capita, sized by
population and colored by region. The LOESS smoother traces the rise–fall–rise:
...
region fixed effects and standard errors clustered by country:
